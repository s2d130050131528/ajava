...
<body><c:set var="id" value="${param.id}"/>
<c:set var="name" value="${param.bookname}"/>
<c:set var="author" value="${param.author}"/>
<sql:setDataSource var="con" driver="com.mysql.jdbc.Driver" url="jdbc:mysql://localhost:3306/s2d130050131528" user="root" password=""/>
<sql:update dataSource="${con}" var="count">
insert into bookinfo(id,bookname,author) values(?,?,?); 
<sql:param value="${id}"/><sql:param value="${name}"/>
<sql:param value="${author}"/></sql:update>
<sql:query dataSource="${con}" var="rs">select * from bookinfo;</sql:query>
<h1 align="center">Book Details</h1>
<table align="center" border="1">
<tr><th>Id</th><th>Book Name</th><th>Author</th></tr>
<c:forEach var="row" items="${rs.rows}">
<tr><td><c:out value="${row.id}"/></td>
<td><c:out value="${row.bookname}"/></td>
<td><c:out value="${row.author}"/></td></tr></c:forEach></table></body> </html>
